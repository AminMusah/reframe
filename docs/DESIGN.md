...
| LLM       | Vercel AI SDK v7 (`ai`, `@ai-sdk/anthropic`, `@ai-sdk/openai`). Provider inferred from the key prefix (`sk-ant-` → Anthropic, else OpenAI). Dropdown per provider: Sonnet 5 (default) / Opus 5 / Haiku 4.5, or GPT-5.6 Terra (default) / Sol / Luna. Choice remembered in `localStorage`, pinned to `interviews.model` when Reframe is clicked; the brief uses its interview's model; a model from the wrong provider for the key falls back to that provider's default (`lib/llm/provider.ts`).                                                  |
