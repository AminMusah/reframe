...
_Agreed 2026-09-12. Reframe turns a freeform diagram into a coding-agent prompt by interviewing the author about what they drew, rather than by constraining them to a typed component palette._
...
- **Containers:** frames are top-level containers named by their label. Inside a frame (or at root), build a geometric containment tree (B is inside A if ≥ 90 % of B's box is within A's).
...
- **Freedraw:** summarized ("scribble near r3"), or "stroke through r3" when it crosses an element — worth a question.
- **Sticky notes:** emitted as author notes.
- **Images / embeds:** noted by type and position only.
- **IDs:** short stable tokens (`r1`, `r2`, `a1`, `t1`, `f1`) mapped to Excalidraw IDs client-side. The model only ever sees the short IDs.
- **Coordinates:** normalized to a coarse 0–100 grid per frame, plus relative words ("below r3", "right of r1"). Never raw pixels.

Fixtures in `fixtures/*.excalidraw` drive Vitest for the serializer and the interview evals (below).
...
- Each question cites `elementIds`; the client selects them and calls `scrollToContent(..., { fitToViewport })`.
...
- `elementIds` returned by the model are validated against the graph's IDs; unknown ones are dropped.
...
- **Projects:** `/` with no `?p=` opens the most-recent project (creating one if none). A "Projects" sheet in the header lists / creates / renames.
