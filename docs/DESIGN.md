...
- **Everything floats.** No drawers, no modals for lists: the Drawings list, the interview panel, Excalidraw's search and its shape library are all undocked Excalidraw sidebars styled as inset cards (search is force-docked by Excalidraw; CSS un-docks it) (`defaultSidebarDockedPreference: false`). Any of them can be pinned.
