...
- **Everything floats.** No drawers, no modals for lists: the Drawings list (left, under the menu button, with the current drawing's name editable in place), the interview panel, Excalidraw's search and its shape library are all undocked Excalidraw sidebars styled as inset cards (search is force-docked by Excalidraw; CSS un-docks it) (`defaultSidebarDockedPreference: false`). Nothing pins: the dock button is hidden everywhere.
