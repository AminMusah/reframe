...
| Concern   | Choice                                                                                                                                                                                                                                                                                                                                              |
| --------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Framework | Next.js 16 (App Router), React 19, Tailwind 4, shadcn — static export (`output: "export"`), no route handlers or server actions. Single page; the project id is `?p=<id>` (dynamic segments are unsupported under export). Route groups: `app/(app)` holds the workspace and its layout mounts the Convex/auth `Providers` (which wait for a session before rendering); `app/(site)` holds plain pages such as `/privacy`, which render without any of that.                                                                                                                          |
...
| Auth      | Better Auth via `@convex-dev/better-auth`, handler served from Convex HTTP actions (`convex/http.ts`), **anonymous plugin** for auto sign-in, GitHub + Google as upgrade. Follow the component's **React (Vite SPA)** guide, not the Next.js one: `registerRoutes(http, createAuth, { cors: true })`, `crossDomain` plugin both sides, client `baseURL` = Convex `.site` URL, OAuth callbacks on `*.convex.site`. **Anonymous → OAuth linking is done by the app** (`users.prepareLink` / `completeLink`, table `pendingLinks`): the plugin's own `onLinkAccount` needs the anonymous session cookie at the OAuth callback — a cross-site navigation to the Convex origin, where the browser has no such cookie (the session lives in localStorage via the crossDomain plugin) — so Better Auth created a fresh user and the visitor's drawings stayed with the orphaned anonymous user (seen on prod, 2026-09-15). Now the anonymous user mints a 15-minute token before the redirect, the browser keeps it, and `Providers` redeems it as the new user before rendering, retrying while the Convex connection still carries the anonymous identity. Navigation home is `useGoHome` (open the most recent drawing directly): on the static export `router.replace("/")` from `/?p=…` did not take and left a blank page. |
...
- **Wrap-up nudge** (2026-09-15). The system prompt's "do not exceed 8" did not hold: a real session on `gpt-5.6-luna` ran 22 questions, drilling into capabilities the author had merely confirmed were wanted (which identity provider, which fields in D1/KV). Two changes: from question `WRAP_UP_AFTER` (8) on, the latest user message carries a line telling the model to return `done` unless a drawn element is still unexplained — a cap in the conversation holds where one in the system prompt does not; and the scope question's answers are *recorded, not drawn* — the model adds a box only for a component the author volunteers, never for an adjacent capability it asked about itself.
- **Edits are justified by the answer.** The same session relabelled five arrows "dispatches jobs" in one sweep, two of them wrongly, and spent two turns undoing it. The prompt now says each op must be justified by the answer just given and an arrow's label changes only when the answer says what that arrow carries.
- **Language.** The first question of that session was in Japanese because the drawing's title was; the author spoke English. The client sends `navigator.language` at `interviews.start` (stored on the interview, so a device switch mid-way does not flip it); the model asks in it whatever language the drawing uses and switches only if the author writes in another. The brief is written in the same language with the section headings kept in English (`nameFromBrief` keys on "# Goal"). Reframe's own UI stays English for now; Excalidraw's chrome (`langCode`) and our strings would be localised together, when non-English users show up.
...
- **Everything floats.** No drawers, no modals for lists: the Drawings list (left, under the menu button, with the current drawing's name editable in place), the interview panel, Excalidraw's search and its shape library are all Excalidraw sidebars styled as floating inset cards. Our two are declared `docked` — Excalidraw closes an *undocked* sidebar on any canvas click, which is unusable while drawing during an interview — and CSS gives docked sidebars the same floating look and cancels the editor-UI shift. The dock button is hidden everywhere.
...
- **Phones and tablets.** Below 640 px every sidebar is full-screen; the top-right controls become icons (the interview badge becomes a dot); on coarse pointers the 1–5 key hints are hidden and option rows grow. A drawing is fitted to the viewport on load when the screen is narrower than 900 px or the scene has no saved scroll.
- **The canvas is the whole window** (v1.3). There is no app header. Everything else lives in Excalidraw's own slots (`components/canvas/chrome.tsx`):
  ...
  - Welcome screen on an empty canvas: wordmark, one-line pitch, *Load an example drawing* (`public/examples/signup-flow.excalidraw`, a six-box sign-up flowchart with one decision — short, obvious gaps, arrow labels; also `fixtures/signup-flow` for evals), *How it works* (opens the panel), hints pointing at the toolbar and menu.
- **The interview panel is an Excalidraw `Sidebar`** (`name="reframe"`, 420 px, floating over the canvas as an inset card; nothing docks (the pin is hidden everywhere)). Docked, Excalidraw shifts its UI and `setViewport({offsets:{ui:true}})` keeps cited elements out from under it; on narrow screens Excalidraw overlays it instead. It opens on its own when a project has an interview underway.
...
- Naming: a drawing is "Untitled" until it names itself from its largest free-standing text (`lib/scene-title.ts`, applied in `saveScene`) or, failing that, from the prompt's Goal sentence when the first prompt completes (`nameFromBrief` in `convex/briefs.ts`); the author can type a name any time in the Drawings panel's "This drawing" field (or ☰ → Rename). Deleting a project cascades to its files, interviews and briefs.
...
- **The prompt is rendered Markdown** via **Streamdown** (`components/markdown.tsx`): GFM, per-block memoisation, and `parseIncompleteMarkdown` so a flush that ends inside `**bold` or an open fence renders finished instead of flickering. Styling is a `components` map hoisted to module scope (inline it would remount the tree on every flush) using our tokens, not Streamdown's stylesheet; `controls={false}`. Fences get a borderless mono block with a language tag and a Copy button — no syntax colour, the design has none to spend. Below it: a word count, Write it again, Interview again and Copy prompt → ✓ Copied (copies the raw Markdown).
...
- **Motion** (`globals.css`): every `[data-slot=button]` and `.pressable` scales to 0.97 on press over 160 ms with a strong ease-out; new cards use `.enter` (`@starting-style`, 6 px rise, 200 ms) and option lists `.enter-stagger` (40 ms steps); the streaming caret is a blinking bar. Transform/opacity only, nothing over 300 ms, keyboard answers do not animate, and `prefers-reduced-motion` disables all of it. Pass of 2026-09-15 (the design-engineering checklist): buttons transition named properties, not `all`, and have one press effect (the scale — the shadcn 1 px nudge is gone); dialogs enter over 200 ms on the strong ease-out from scale .95 and leave faster (120 ms, scale .98), overlay 60 % not 80 %; the floating panels arrive over 200 ms from 8 px beside their place, Excalidraw's menus scale in over 150 ms from the corner they hang off (never from nothing), and its toolbar tools answer a press with the same scale; the spinner turns in 0.7 s; a swapped state (Copy → Copied) crossfades through 2 px of blur (`.swap`, keyed) instead of snapping. Tooltips are unused, so no delay-skip logic exists.
...
**Breathing room after every edit** (`lib/edits/tidy.ts`, 2026-09-14). The converter wraps a label to whatever width the box had and grows it downward, and nothing else moves — a renamed box landed on its neighbour, a long arrow label hid under a shape, and a new arrow ran straight through whatever was in between (reproduced with one edit on the three-tier fixture). Rather than asking the model to lay things out, `applyEdit` runs a deterministic pass over what the edit touched: (1) a changed box is sized for its label — one line up to 320 px, else a few lines — keeping its centre; (2) a labelled arrow gets enough length for its label, which wraps beyond 200 px, by moving the end the edit brought in (never a box that just grew; at most 260 px); (3) anything a grown or moved box now overlaps is pushed away along the pusher's direction of travel, cascading, and containers (shapes holding others, frames) grow to keep their children while non-children are pushed off their borders; (4) arrows bound to moved boxes are re-anchored, and a new straight arrow that would cross another box is routed over or under it as an elbow. Steps 2–3 run twice so one fix does not undo another. Lessons from a real session (2026-09-14, the sign-up example): `add … below X` now places the box *adjacent* and lets the push cascade insert a row (the old collision-dodging skipped past everything to the bottom of the drawing); a box the edit just added never counts as a container's child (dropped onto a diamond it was being treated as "inside" it and left there); boxes only ever grow (a box that merely gained an arrow was being shrunk to its label); free-standing text is pushed like a box; pushes follow the pusher's travel only for boxes ahead of it; a hand-drawn multi-point arrow's ends stay on the same edge of their box (fixedPoints are centres in orbit mode); straight-in-practice polylines are re-anchored like straight arrows; bent arrows' labels get extra room. In development `window.__reframe` exposes `applyEdit`/`serializeScene`/the API so an op sequence can be replayed against the live canvas headlessly. Text is measured with a canvas in the browser (fallback 0.6 em per character). Undo still restores the pre-edit snapshot, so the pass is never a one-way door.

**Check the work, then lay it all out if needed** (`lib/edits/layout.ts`, 2026-09-15). The tidy pass only makes room locally, and a real session (the sign-up drawing after five agent edits) still ended with arrows through boxes, labels on shapes and a box dropped onto an arrow — the author had to click a label to read it. So after tidying, `applyEdit` **audits** the drawing (box–box overlaps under 8 px, an arrow segment through a box it is not bound to, an arrow label over a box) and, if anything is wrong, lays the whole drawing out again as a layered graph with **dagre** (`@dagrejs/dagre`): boxes keep their sizes, arrow labels reserve their space in the layout, the flow keeps its direction (down or right, whichever most arrows do), nodes enter in the author's reading order so ties keep it, and the drawing keeps its top-left corner so a title stays a title. Arrows become straight-legged polylines from dagre's route, squared off so they arrive vertically or horizontally; a diamond or ellipse is entered at the side midpoint facing the other box. The change pill says "… — and tidied the whole drawing" and the viewport fits the result; Undo restores the snapshot. The same pass is the main menu's **Tidy up the drawing** (one undoable step). Drawings with frames or boxes inside boxes are left to the tidy pass alone (dagre's compound support is not wired). Fixture: `fixtures/messy-signup.excalidraw`, the real scene. When the pass has to bow out, what is still wrong travels to the model with the next answer in words ("the label a3 'validate' sits on r5 …") and a new **`move`** op (`{op:"move", id, place:{relative, of}}`, applied inside the tidy pass so arrows follow and neighbours are pushed) lets it fix placement itself — the tldraw agent starter kit's "lint, then prompt again" loop, with the deterministic pass first.

**Sketch from description** (2026-09-15). A tester cleared the canvas and typed "draw a cat"; the model returned a `sketch` turn, but the sketcher only knew how to redraw a picture on the canvas — with no picture it found "no diagram" and the author undid it. Now `sketchActions.fromReference` takes an optional PNG: with a picture on the canvas the client photographs it as before; without one it sends the instruction alone and `sketchFromDescription` (`DESCRIBE_PROMPT`) lays the diagram out from words on the same grid schema. That is also how a first diagram gets onto an empty canvas — edit ops need an anchor, a sketch does not — and the interview prompt says so. Non-diagrams are drawn simply from the shapes available rather than refused; in `sketch-apply`, an unlabelled shape is treated as pure drawing: it keeps its grid geometry (no label floor) and may touch its neighbours, so a cat's eyes stay in its head. The model is also told which model it is running as; testers kept asking.
